...
export default function PraiseDashboard() {
  ...
  async function updateRating(id, newRating) {
    setTracks(
      tracks.map((t) => (t.id === id ? { ...t, rating: newRating } : t))
    );
    await supabase
      .from("playlist_tracks")
  ...
  async function runNARAnalysis(track) {
    ...
    try {
      ...
      setTracks(
        tracks.map((t) => (t.id === track.id ? { ...t, ...updates } : t))
      );
  ...
  return (
    <div className="p-8 bg-gray-900 min-h-screen text-white relative">
      ...
      <div className="bg-gray-800 p-4 rounded-lg border border-gray-700 mb-8 flex flex-wrap gap-4 items-center">
        ...
        <select
          value={filterSource}
          // FIX: Changed setFilterNar to setFilterSource
        ...
        >
          ...
          <option value="KLOVE">KLOVE</option>
          <option value="The Light FM">The Light FM</option>
        </select>
      ...
      <div className="grid gap-4">
        {tracks.map((track) => (
          <div
            key={track.id}
            className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden flex flex-col transition-all"
          >
              ...
              <div className="flex-1 flex gap-4 items-start">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {track.nar_rating && (
                      <span
                        className={`text-xs px-2 py-1 rounded font-bold ${getNarBadgeColor(
                          track.nar_rating
                        )}`}
                      >
